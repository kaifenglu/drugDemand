...
f_drug_description <- function(j) {
  conditionalPanel(
    condition = paste("input.l2 ==", j),

    shinyMatrix::matrixInput(
      paste0("drug_description_", j),
      label = "Drug name",
      value = matrix(paste("Drug", 1:j), nrow = j, ncol = 1),
      inputClass = "character",
      rows = list(names=FALSE, extend=FALSE),
      cols = list(names=FALSE, extend=FALSE)))
}


f_kit_description <- function(j) {
  conditionalPanel(
    condition = paste("input.l ==", j),

    shinyMatrix::matrixInput(
      paste0("kit_description_", j),
      label = "Kit name",
      value = matrix(paste("Kit", 1:j), nrow = j, ncol = 1),
      inputClass = "character",
      rows = list(names=FALSE, extend=FALSE),
      cols = list(names=FALSE, extend=FALSE)))
}
...
f_kit_probability <- function(j) {
  conditionalPanel(
    condition = paste("input.l ==", j, "&& input.stage == 'Design stage'"),

    shinyMatrix::matrixInput(
      paste0("kit_probability_", j),
      label = "Unnormalized prior probabilities of kits within a drug",
      value = matrix(1, nrow = j, ncol = 1,
                     dimnames = list(paste("Kit", 1:j), NULL)),
      inputClass = "numeric",
      rows = list(names=TRUE, extend=FALSE),
      cols = list(names=FALSE, extend=FALSE)))
}
...
f_drug_by_kit <- function(i, j) {
  conditionalPanel(
    condition = paste("input.l2 ==", i, "&& input.l ==", j),

    shinyMatrix::matrixInput(
      paste0("drug_by_kit_", i, "_", j),
      label = "Potential kit types for each drug",
      value = matrix(1, nrow = i, ncol = j,
                     dimnames = list(paste("Drug", 1:i), paste("Kit", 1:j))),
      inputClass = "numeric",
      rows = list(names=TRUE, extend=FALSE),
      cols = list(names=TRUE, extend=FALSE)))
}
...
f_treatment_by_drug <- function(i, j) {
  conditionalPanel(
    condition = paste("input.k ==", i, "&& input.l2 ==", j),
...
f_dosing_schedule <- function(j) {
  conditionalPanel(
    ...
    shinyMatrix::matrixInput(
      ...
      value = matrix(rep(c(21, 2, 10000), each = j), nrow = j, ncol = 3,
                     dimnames = list(paste("Kit", 1:j),
...
eventPanel <- tabPanel(
  ...
  conditionalPanel(
    ...
    fluidRow(
      ...
      column(8, lapply(1:6, f_exponential_survival),
...
dosingSchedulePanel <- tabPanel(
  ...
  fluidRow(
    column(6, selectInput(
      "l2", label = "Total number of drugs",
      choices = seq_len(12), selected = 2)),

    column(6, selectInput(
      "l", label = "Total number of kit types",
      choices = seq_len(12), selected = 2))
  ),

  fluidRow(
    column(6, lapply(1:12, f_drug_description)),
    column(6, lapply(1:12, f_kit_description))
  ),

  lapply(1:12, f_kit_probability),

  lapply(c(outer(1:12, 1:12, function(i,j) 1000*i+j)),
         function(k) f_drug_by_kit(k %/% 1000, k %% 1000)),
...
server <- function(input, output, session) {
  ...
  df <- reactive({
    ...
    if (input$by_treatment) {
      req_cols <- c(req_cols, 'treatment', 'treatment_description')
    }
  ...
  visitview <- reactive({
    ...
    req_cols <- c('usubjid', 'date', 'drug', 'drug_name', 'kit',
                  'kit_name', 'dispensed_quantity')
    ...
    tibble(df) %>% mutate(date = as.Date(date))
  })
  ...
  observeEvent(list(input$stage, to_predict(), predict_dosing()), {
    if (input$stage != 'Design stage' &&
        to_predict() != 'Enrollment only' && predict_dosing()) {
  ...
  output$cum_dose_t0 <- renderPrint({
    if (!is.null(dose_observed())) {
      ...
      colnames(table) <- c("Kit", "Kit Name", "Cumulative Dose")
  ...
  l <- reactive({
    if (input$stage != "Design stage" && !is.null(visitview())) {
      l = nrow(dose_observed()$kit_description_df)
  ...
  l2 <- reactive({
    if (input$stage != "Design stage" && !is.null(visitview())) {
      l2 = length(unique(dose_observed()$kit_description_df$drug))
      updateSelectInput(session, "l2", selected=l2)
    } else {
  ...
  drug_name <- reactive({
    ...
    if (input$stage != "Design stage" && !is.null(visitview())) {
      df <- dose_observed()$kit_description_df %>%
        group_by(drug, drug_name) %>%
        slice(1)
      drug_name = df$drug_name
    } else {
      param = input[[paste0("drug_description_", l2())]]
  ...
  kit_name <- reactive({
    ...
    if (input$stage != "Design stage" && !is.null(visitview())) {
      kit_name = dose_observed()$kit_description_df$kit_name
    } else {
      param = input[[paste0("kit_description_", l())]]
      kit_name = as.character(param[,1])
    }
  ...
  kit_probability <- reactive({
    req(l())

    if (input$stage == "Design stage") {
      param = input[[paste0("kit_probability_", l())]]
      t = as.numeric(param[,1])

      valid = all(t > 0)
      if (!valid) {
        showNotification(
          "Unnormalized prior kit probaibilities must be positive"
        )
      }
    } else {
      t = rep(1, l())
  ...
  drug_by_kit <- reactive({
    req(l2())
    req(l())

    if (input$stage != 'Design stage' && !is.null(visitview())) {
      kit_description_df <- dose_observed()$kit_description_df

      drug_by_kit <- matrix(0, nrow = l2(), ncol = l(),
                            dimnames = list(drug_name(), kit_name()))
      for (i in 1:nrow(kit_description_df)) {
        drug_by_kit[kit_description_df$drug[i],
                    kit_description_df$kit[i]] = 1
      }
    } else {
      param = input[[paste0("drug_by_kit_", l2(), "_", l())]]
      t = as.numeric(param)

      valid = all(t==1 | t==0)
      if (!valid) {
        showNotification(
          "Entries of drug by kit matrix must be 1 or 0"
        )
      }

      req(valid)

      drug_by_kit <- matrix(t, nrow = l2(), ncol = l(),
                            dimnames = list(drug_name(), kit_name()))
    }
  ...
  kit_description_df <- reactive({
    t = as.numeric(drug_by_kit())

    df <- tibble(drug = rep(1:l2(), l()),
                 drug_name = rep(drug_name(), l()),
                 kit = rep(1:l(), each=l2()),
                 kit_name = rep(kit_name(), each=l2()),
                 included = as.logical(t)) %>%
      filter(included) %>%
      select(drug, drug_name, kit, kit_name)

    if (input$stage == 'Design stage') {
      df$p_kit = kit_probability()
    }
  ...
  treatment_by_drug <- reactive({
    ...
    if (input$stage != 'Design stage' && !is.null(visitview())) {
      treatment_by_drug_df <- dose_observed()$treatment_by_drug_df

      treatment_by_drug <- matrix(0, nrow = k(), ncol = l2(),
                                  dimnames = list(treatment_description(),
                                                  drug_name()))
      ...
      updateMatrixInput(
        session, paste0("treatment_by_drug_", k(), "_", l2()),
        value=matrix(treatment_by_drug, nrow = k(), ncol = l2(),
                     dimnames = list(treatment_description(), drug_name())))
    } else {
      param = input[[paste0("treatment_by_drug_", k(), "_", l2())]]
      t = as.numeric(param)
      ...
      treatment_by_drug <- matrix(t, nrow = k(), ncol = l2(),
                                  dimnames = list(treatment_description(),
                                                  drug_name()))
    }
  ...
  treatment_by_drug_df <- reactive({
    ...
    tibble(treatment = rep(1:k(), l2()),
           drug = rep(1:l2(), each=k()),
           included = as.logical(t)) %>%
      filter(included) %>%
      select(treatment, drug)
  })
  ...
  dosing_schedule_df <- reactive({
    ...
    df <- tibble(
      kit = 1:l(),
      target_days = as.numeric(param[, "Days per Cycle"]),
      target_dose = as.numeric(param[, "Dose per Cycle"]),
      max_cycles = as.numeric(param[, "Number of Cycles"]))

    valid1 = all(df$target_days > 0 & df$target_days == round(df$target_days))
    if (!valid1) {
      showNotification("Days per Cycle must be positive integers")
    }

    valid2 = all(df$target_dose >= 0)
    if (!valid2) {
      showNotification("Dose per Cycle must be nonnegative")
    }

    valid3 = all(df$max_cycles > 0 & df$max_cycles == round(df$max_cycles))
    if (!valid3) {
  ...
  observeEvent(list(drug_name(), l2()), {
    updateMatrixInput(
      session, paste0("drug_description_", l2()),
      value=matrix(drug_name(), nrow = l2(), ncol = 1))
  })


  observeEvent(list(kit_name(), l()), {
    updateMatrixInput(
      session, paste0("kit_description_", l()),
      value=matrix(kit_name(), nrow = l(), ncol = 1))
  })


  observeEvent(list(kit_name(), l()), {
    updateMatrixInput(
      session, paste0("kit_probability_", l()),
      value=matrix(kit_probability(), nrow = l(), ncol = 1,
                   dimnames = list(kit_name(), NULL)))
  })


  observeEvent(list(drug_name(), kit_name(), l2(), l()), {
    updateMatrixInput(
      session, paste0("drug_by_kit_", l2(), "_", l()),
      value=matrix(drug_by_kit(), nrow = l2(), ncol = l(),
                   dimnames = list(drug_name(), kit_name())))
  })


  observeEvent(list(treatment_description(), drug_name(), k(), l2()), {
    updateMatrixInput(
      session, paste0("treatment_by_drug_", k(), "_", l2()),
      value=matrix(treatment_by_drug(), nrow = k(), ncol = l2(),
                   dimnames = list(treatment_description(), drug_name())))
  ...
  model_k0 <- reactive({
    if (input$stage != 'Design stage' && !is.null(df()) &&
      ...
      vf <- dose_observed()$vf %>%
        left_join(dosing_schedule_df(), by = "kit")
    ...
    if (!is.null(dose_observed()) & !is.null(dosing_schedule_df())) {
      ...
      if (common_time_model()) {
        ...
        for (h in 1:l()) {
          ...
          k0_fit[[h]] <- f_fit_ki(df_k0, model_k0(), nreps(),
                                  showplot = FALSE)

          k0_fit[[h]]$fit_plot <- f_kit_name(
            k0_fit[[h]]$fit_plot, kit_name(), h)
        }
      }
    ...
    if (!is.null(dose_observed()) & !is.null(dosing_schedule_df())) {
      ...
      if (common_time_model()) {
        ...
        for (h in 1:l()) {
          ...
          t0_fit[[h]]$fit_plot <- f_kit_name(
            t0_fit[[h]]$fit_plot, kit_name(), h)
        }
      }
      ...
      if (common_time_model()) {
        ...
        for (h in 1:l()) {
          ...
          if (nrow(df_t1) == 0) {
            ...
            t1_fit[[h]]$fit_plot <- f_kit_name(
              t1_fit[[h]]$fit_plot, kit_name(), h)
          }
        }
      }
    ...
    if (!is.null(dose_observed()) & !is.null(dosing_schedule_df())) {
      ...
      if (common_time_model()) {
        ...
        for (h in 1:l()) {
          ...
          ki_fit[[h]] <- f_fit_ki(df_ti, model_ki(), nreps(),
                                  showplot = FALSE)

          ki_fit[[h]]$fit_plot <- f_kit_name(
            ki_fit[[h]]$fit_plot, kit_name(), h)
        }
      }
    ...
    if (!is.null(dose_observed()) & !is.null(dosing_schedule_df())) {
      ...
      if (common_time_model()) {
        ...
        for (h in 1:l()) {
          ...
          ti_fit[[h]] <- f_fit_ti(df_ti, model_ti(), nreps(),
                                  showplot = FALSE)

          ti_fit[[h]]$fit_plot <- f_kit_name(
            ti_fit[[h]]$fit_plot, kit_name(), h)
        }
      }
  ...
  di_fit <- reactive({
    if (!is.null(dose_observed()) & !is.null(dosing_schedule_df())) {
      ...
      for (h in 1:l()) {
        ...
        di_fit[[h]]$fit_plot <- f_kit_name(
          di_fit[[h]]$fit_plot, kit_name(), h)
      }
    ...
    if (!is.null(dosing())) {
      ...
      if (input$stage == 'Design stage') {
        ...
        g <- purrr::map(1:l(), function(h) {
          dfa <- dfs %>%
            filter(kit == h & parameter == "observed data")
          dfb <- dfs %>%
            filter(kit == h & parameter == "model based prediction")
          dfb_pp <- dfs %>%
            filter(kit == h & parameter == "protocol based prediction")
          ...
          fig <- f_kit_name(fig, dfa$kit_name, 1)
  ...
  output$saveInputs <- downloadHandler(
    ...
    content = function(file) {
      x <- list(
        ...
        l = l(),
        l2 = l2(),
        drug_description = matrix(drug_name(), ncol = 1),
        kit_description = matrix(kit_name(), ncol = 1),
        kit_probability = matrix(kit_probability(), ncol = 1,
                                 dimnames = list(kit_name(), NULL)),
        drug_by_kit = drug_by_kit(),
        treatment_by_drug = treatment_by_drug(),
        dosing_schedule = matrix(
          input[[paste0("dosing_schedule_", l())]], ncol = 3,
          dimnames = list(kit_name(), c("Days per Cycle", "Dose per Cycle",
                                        "Number of Cycles"))),
        model_k0 = model_k0(),
        model_t0 = model_t0(),
        model_t1 = model_t1(),
  ...
  observeEvent(input$loadInputs, {
    ...
    if (x$predict_dosing) {
      ...
      updateMatrixInput(
        session, paste0("drug_description_", x$l2),
        value=x$drug_description)

      updateMatrixInput(
        session, paste0("kit_description_", x$l),
        value=x$kit_description)

      if (x$stage == 'Design stage') {
        updateMatrixInput(
          session, paste0("kit_probability_", x$l),
          value=x$kit_probability)
      }

      updateMatrixInput(
        session, paste0("drug_by_kit_", x$l2, "_", x$l),
        value=x$drug_by_kit)

      updateMatrixInput(
        session, paste0("treatment_by_drug_", x$k, "_", x$l2),
