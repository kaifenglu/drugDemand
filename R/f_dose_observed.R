...
#'
#' * \code{drug_description_df}: A data frame indicating the
#'   drug and kit descriptions, including the following variables:
#'   \code{drug}, \code{drug_name}, \code{dose_strength}, \code{kit},
#'   \code{kit_name}, and \code{dose_unit}.
#'
...
f_dose_observed <- function(df = NULL, visitview = NULL, showplot = TRUE) {
  ...
  vf <- visitview %>%
    ...
    group_by(.data$drug, .data$drug_name, .data$dose_strength,
             .data$dose_unit, .data$usubjid, .data$treatment,
    ...
    group_by(.data$drug, .data$drug_name, .data$dose_strength) %>%
    mutate(kit = cur_group_id(),
           kit_name = trimws(paste(.data$drug_name,
                                   .data$dose_strength))) %>%
    group_by(.data$drug, .data$drug_name, .data$dose_strength,
  ...
  dosing_summary_t0 <- dosing_summary_t %>%
    ...
    select(c("kit", "kit_name", "dose_unit", "cum_dose_t0"))
  ...
  bar_t0_df <- bar_t0_df %>%
    left_join(drug_description_df, by = "kit")
